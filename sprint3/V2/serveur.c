...
void send_help_msg(int dS) {
    ...
    if (file == NULL) {
        perror("Erreur lors de l'ouverture du fichier de fonctionnalités");
        return;
    }
    char line[256];
...
/* Thread de réception de fichiers */
void *file_transfer_handler(void *arg) {
    int file_socket = *(int *)arg;
    char buffer[512];
    receive(file_socket, buffer, sizeof(buffer));

    char filename[256];
    long filesize;
    sscanf(buffer + 5, "%s %ld", filename, &filesize);
    receive_file(file_socket, filename, filesize);
    ...
    while (!isEnd) {
        ...
        if (!isEnd) {
            if (strcmp(msgReceived, "help\n") == 0) {
                ...
                // La commande file est gérée par le thread file_transfer_handler
...
void *file_listener(void *arg) {
    ...
    while (s_r) {
        int new_file_socket = accept(file_socket, NULL, NULL);
        if (new_file_socket == -1) {
            perror("erreur au accept du socket de transfert de fichiers");
            continue;
        }
        pthread_create(&file_thread, NULL, file_transfer_handler, &new_file_socket);
...
int main(int argc, char *argv[]) {
    ...
    // Configurer le socket de message
    ...
    if (bind(dS, (struct sockaddr*)&ad, sizeof(ad)) == -1) {
        perror("erreur au bind du socket de message");
        exit(-1);
    }
    if (listen(dS, MAX_CLIENT) == -1) {
        perror("erreur au listen du socket de message");
        exit(-1);
    }

    // Configurer le socket de transfert de fichiers
    ...
    if (bind(file_socket, (struct sockaddr *)&file_addr, sizeof(file_addr)) == -1) {
        perror("erreur au bind du socket de transfert de fichiers");
        exit(-1);
    }
    if (listen(file_socket, MAX_CLIENT) == -1) {
        perror("erreur au listen du socket de transfert de fichiers");
        exit(-1);
    }
    ...
    pthread_t message_thread, file_thread;
    if (pthread_create(&message_thread, NULL, message_listener, &dS) != 0) {
        perror("Erreur lors de la création du thread d'écoute des messages");
        exit(EXIT_FAILURE);
    }
    if (pthread_create(&file_thread, NULL, file_listener, &file_socket) != 0) {
        perror("Erreur lors de la création du thread d'écoute des fichiers");
        exit(EXIT_FAILURE);
    }
